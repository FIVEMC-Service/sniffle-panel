#!/bin/bash
# WebPanel Auto-Installer mit korrigiertem Passwort-Handling
# Version: 3.1
...
install_panel() {
  ...
  # Hier wird das Passwort direkt als String eingefügt
...
// Konfiguration
\$ADMIN_USER = 'admin';
\$ADMIN_PASS = '${ADMIN_PASS}';  // Direktes Einfügen des Passworts
...
if (isset(\$_POST['add_domain']) && isset(\$_SESSION['loggedin'])) {
    ...
    // Apache aktivieren
    shell_exec("a2ensite {$full_domain}.conf");
    shell_exec("systemctl reload apache2");
    
    // SSL installieren
    shell_exec("certbot --apache -d {$full_domain} --non-interactive --agree-tos --email ${ADMIN_EMAIL}");
